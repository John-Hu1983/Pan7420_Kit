...
#define EXAMPLE_NAME	"PN298 normal mode compatible with xn297l"
...
//			BSP_LED_Toggle(LED_GREEN);
			...
			/* 发送数据，每次发送32个字节 */
			bsp_pn298_tx_data(temp_buff, __PN298_PAYLOAD);
			printf("Tx cnt:%d\r\n", ++count);
		}
		
		/* 如果PN298工作于接收模式，进入如下程序 */
		if(tx_rx_mode == 0) {
			if(bsp_pn298_rx_data(temp_buff, __PN298_PAYLOAD)){
				printf("Rx cnt:%d\r\n", count++);
			}	
		}
